...
class MediaListWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Obx(() =>
         (mediaListController.mediaList.isEmpty) ? Center(
            child: Text(
              Resources.noMedia,
        ...
        : GridView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              ...
              bool isMediaVideo = mediaListController.mediaList[index].contains(Constants.mp4) ||
                  mediaListController.mediaList[index].contains(Constants.mov);
              IconData icon =
                      ...
                      onTap: () {
                        showDialog(
                            ...
                            builder: (context) => isMediaVideo
                                ? VideoPlayerScreen(media: mediaListController.mediaList[index])
                                : AudioPlayerScreen(media: mediaListController.mediaList[index]));
                      },
                      child: Center(
                          child: Icon(
                            icon,
                      ...
                      onPressed: () {
                        showDialog<String>(
                          ...
                          builder: (BuildContext context) => AlertDialog(
                            content: Text(Resources.confirmDeletingElement),
                            actions: <Widget>[
                              TextButton(
                              ...
                              TextButton(
                                onPressed: () async {
                        Get.find<MediaListController>().deleteMediaInLocalStorage(mediaListController.mediaList[index]);
                        Get.back();
                        final snackBar = SnackBar(
                          backgroundColor: MediaColors.snackbarSuccess,
                          content: Text(Resources.textElementDeleted),
                        );
